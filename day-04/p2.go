...
func main() {
	...
	lastWinningBoardIndex := winOrder[len(winOrder)-1]

	lastWinningBoard := boards[lastWinningBoardIndex]
	for _, vector := range found[lastWinningBoardIndex] {
		lastWinningBoard[vector[0]][vector[1]] = 0
	}

	var sum int
	for _, row := range lastWinningBoard {
	...
	log.Printf("Last number called: %d\n", winners[lastWinningBoardIndex])

	fmt.Printf("Final score: %d\n", sum*winners[lastWinningBoardIndex])
